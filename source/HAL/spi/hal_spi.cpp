...
SPIBase::SPIBase(SPI_TypeDef* spi_peripheral_address, gpio::output_pin& chip_select)
...
void SPIBase::write_control_register(SPIControlRegister1 reg, uint8_t value) {
    ...
    m_peripheral->CR1 = m_peripheral->CR1 & ~static_cast<uint16_t>((register_mask << static_cast<uint8_t>(reg)));
    m_peripheral->CR1 = m_peripheral->CR1 | static_cast<uint16_t>(((value & register_mask) << static_cast<uint8_t>(reg)));
}

void SPIBase::write_control_register(SPIControlRegister2 reg, uint8_t value) {    
    m_peripheral->CR2 = m_peripheral->CR2 & ~static_cast<uint16_t>((static_cast<uint8_t>(0x01) << static_cast<uint8_t>(reg)));
    m_peripheral->CR2 = m_peripheral->CR2 | static_cast<uint16_t>(((value & 0x01) << static_cast<uint8_t>(reg)));
}
...
void SPIPolling::read_write(uint8_t* tx_buffer, uint8_t* rx_buffer, uint16_t size) {     
    ...
    while ( size  ) {
        ...
        m_peripheral->DR = (tx_buffer != nullptr) ? *tx_buffer++ : static_cast<uint8_t>(0x00);
